...
from video2dataset.subsamplers.clipping_subsampler import _get_seconds
    ...
    def __call__(self, stream: FFmpegStream, tmpdir: str, metadatas: List[Metadata]) -> Tuple[FFmpegStream, List[Metadata], Error]:
        if self.downsample_method == "fps":
            ...
            for frame_id, start_t in enumerate(starts):
                ...
                stream = ffmpeg.input(f"{tmpdir}/input.mp4", ss=start_t)
                stream = stream.output(f"{tmpdir}/frame_{frame_id}.jpg", vframes=1, **{"q:v": 2}).run(
                    capture_stdout=True, quiet=True
                )
                with open(f"{tmpdir}/frame_{frame_id}.jpg", "rb") as f:
                    subsampled_bytes.append(f.read())
                metadatas.append(meta_frame)
